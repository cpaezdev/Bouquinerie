...
import { takeOneOrder } from "../../../api/order"

import { Link } from "react-router-dom"
import moment from "moment"
import { config } from "../../../config"

const OrderDetailsAdm = (props) => {
    const [order, setOrder] = useState(null)

    useEffect(() => {
        takeOneOrder(props.params.id)
            .then((res) => {
                if (res.status === 200) {
                    setOrder(res.result)
    ...
    const groupBooksById = (order) => {
        ...
        order.forEach((item) => {
            ...
            // On ajoute l'auteur à la liste d'auteurs pour ce livre
    ...
    return (
        <section className="order-display">
            ...
            <article>
                ...
                {order !== null && (
                    <p>Commande du {moment(order[0].date).format("DD-MM-YYYY")}</p>
                )}
                <table>
                    <thead>
                        <tr>
                    ...
                    {order !== null && (
                        <tfoot>
                            ...
                            <tr>
                                <td colSpan={5}>Poids total des livres: {order[0].total_weight} g</td>
                            </tr>
                            <tr>
                                <td colSpan={5}>Frais de livraison: {order[0].costs} €</td>
                            </tr>
                            <tr>
                                <td colSpan={5}>Total de la commande: {order[0].total_amount} €</td>
                            </tr>
                        </tfoot>
                    )}
                </table>
